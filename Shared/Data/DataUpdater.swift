...
class DataUpdater {
    ...
    func downloadAllDataToFiles() async -> DownloadResult {
        let fileNames = Set([
            files.news, files.areas, files.artists, files.events, files.stages,
            files.tags,
        ])
        ...
        for fileName in fileNames {
            ...
            let downloadResult = await downloadFile(
                url: url,
                destination: getCacheUri(for: fileName)
            )
    ...
    func downloadFile(url: URL, destination: URL, overwrite: Bool = true) async
        -> DownloadResult
    {
        print("Downloading from \(url)")

        let result: DownloadResult = await withCheckedContinuation {
            continuation in
